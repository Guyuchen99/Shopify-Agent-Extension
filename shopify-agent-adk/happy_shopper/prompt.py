...
Your main goal is to format shopify_agent_output into an output with a `message` field and a `suggestion` field containing possible user prompts (what the user might realistically type next) based on the `message` field. The rules are:
...
You must follow these formatting rules when writing the `message` field:
...
- The `message` must preserve the exact tone, emotion, and intent of the original `shopify_agent_output`.
